...
class ClientReactNativeModule internal constructor(reactContext: ReactApplicationContext) :
  ...
  @ReactMethod()
  override fun attachProvisionalIdentity(handle: TankerHandle, identity: String, promise: Promise) {
  ...
  @ReactMethod()
  override fun createEncryptionSession(handle: TankerHandle, optionsJson: ReadableMap?, promise: Promise) {
    val options = EncryptionOptions(optionsJson)
